...
public class DunGen : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for(int i = mainHops-1; i >= 0; i--)
        {
            if (i != mainHops-1)
            {
                mainPoints[mainHops-i-1] = new TurnPoint(mainPoints[mainHops-2-i].nextHop,i,true,Radius,tileLength);
                //Debug.Log(mainPoints[mainHops-i-1].nextHop.x);
            }
            else
            {
                mainPoints[mainHops-i-1] = new TurnPoint(new Vector2Int(0,0),i,true,Radius,tileLength); 
            }
        }
        ...
        for (int j = 0; j < mainHops; j++)
        {
            ...
            if (j!=mainHops-1)
            {
                ...
                int tilesBetweenLength = (int)Mathf.Abs((ntp.position.x-tp.position.x)/tileLength)+(int)Mathf.Abs((ntp.position.y-tp.position.y)/tileLength)+1;
                ...
                for(int i = tp.position.x; nextPointCheck(i,tp.position.x,ntp.position.x); i+=(int)Mathf.Sign(ntp.position.x-tp.position.x)*tileLength)
                {
                    //Debug.Log(i);
                ...
                for(int i = tp.position.y; nextPointCheck(i,tp.position.y,ntp.position.y); i+=(int)Mathf.Sign(ntp.position.y-tp.position.y)*tileLength)
                {
                    //Debug.Log(i);
    ...
    bool nextPointCheck(int pos, int lastpos, int nextpos)
    {
        if (nextpos==lastpos)return false;
        if (lastpos<nextpos)
        {
...
class TurnPoint{
    ...
    public TurnPoint(Vector2Int position,int hopCount, bool isMain, int Radius, int tileLength){
        ...
        if (isMain)
        {
            branchcount = Random.Range(0,3+1);
        }
        if(hopCount>0)
        {
    ...
    Vector2Int newPoint(){
        ...
        nextHopP.x = Random.Range(position.x-Radius,position.x+Radius); 
